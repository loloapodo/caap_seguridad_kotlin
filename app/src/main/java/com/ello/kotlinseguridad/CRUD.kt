...
companion object{
    ...
    suspend fun CargarTodasActividadesDelUsuario(U: Usuario,fg: (list:List<Actividad>) -> Unit,fb: () -> Unit){

        val query = ParseQuery.getQuery<Actividad>(Actividad.class_name)
        query.orderByDescending(Actividad.field_fecha)
        query.whereEqualTo(Actividad.field_rel_usuarios,U)
        query.findInBackground(FindCallback { list, e ->
            if (e==null){Log.e("Tamano lista",list.size.toString());fg(list) }
            else
            { fb();Log.e("Error","Buscar All Actividades") }
        })
    }
    suspend fun CargarTodasActividadesDelUsuario(str_ObjectId: String,fg: (list:List<Actividad>) -> Unit,fb: () -> Unit){
        CargarUnUsuario(str_ObjectId,
                { esteusuario -> GlobalScope.launch {CargarTodasActividadesDelUsuario(esteusuario, fg, fb)} },
                fb);
    ...
    suspend fun RegistrarRespuesta(U: Usuario,F:Formulario,P:Pregunta,str_resp:String,foto: Bitmap?,str_fecha: String,fg: () -> Unit,fb: () -> Unit) {
        val r=Respuesta()
        r.ref_pregunta=P
        r.ref_formulario=F
        r.ref_usuario=U
        r.fecha=str_fecha
        r.respuesta=str_resp
        r.ubicacion//todo
        ...
        fun innerSave(r:Respuesta,fg: () -> kotlin.Unit,fb: () -> Unit){
            r.saveInBackground { e2 ->
                if (e2 == null) {
                    ...
                    fb();Log.e("Error","No registrada respuesta")
                }
            }
        }
   ...
   fun CargarTodosFormulariosRespondidos(u:Usuario,fg: (list:List<Formulario>) -> Unit,fb: () -> Unit){
          ...
          val formRespondidos= mutableListOf<Formulario>()
           for (r in respuestasDelUsuario) {
               if (!formRespondidos.contains(r.ref_formulario)){
                   formRespondidos.add(r.ref_formulario as Formulario)
               }
           }
           fg(formRespondidos)
    ...
    /**
     * Todo terminar esta funciom (restar al todal los formularios los que ya estan respondidos) Implementar un cloudcode quizas
     */
