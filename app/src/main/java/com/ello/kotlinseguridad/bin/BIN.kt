...
companion object {
    ...
    val PIN_INCIDENTE_SELECTED= "editar_inc"
    ...
    val PIN_TODAS_INC = "PIN_TODAS_INC"
    ...
    val PIN_SUFIJO_ACT_RELATION2FORM = "SUF_ACT_REL2FORM"
    ...
    fun PinSelected(p: ParseObject) {
        ...
        if (p is Incidente) {
            ParseObject.unpinAll(PIN_INCIDENTE_SELECTED);p.pin(PIN_INCIDENTE_SELECTED);
        }else
        if (p is Usuario) {
